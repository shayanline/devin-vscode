...
| Reasoning / "thinking" display | Native collapsible section, collapsed by default with a summary label, expandable | Inline italic text, always expanded, not collapsible | 🟡 | User specifically called this out. Wrap thoughts in a collapsible block with a "Thought for Xs" summary and a chevron. Respect `devin.showThinking`. |
